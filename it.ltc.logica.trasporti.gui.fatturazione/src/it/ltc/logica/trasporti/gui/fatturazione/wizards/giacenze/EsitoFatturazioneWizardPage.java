...
import it.ltc.logica.trasporti.gui.elements.fatturazione.TabellaFatturazioneGiacenze;
...
public class EsitoFatturazioneWizardPage extends PaginaWizardRisultati {
	...
	private class ProcessoCaricamentoDati extends ProcessoUI {
		...
		public void eseguiOperazioniSincronizzate() throws RuntimeException {
			...
			tableViewerDettaglio = new TabellaFatturazioneGiacenze(compositeDettaglio, listini.get(0), true);
	...
	private void esportaDati() {
		...
		if (scelta) {
			boolean salvataggio = controllerFatturazione.salvaDatiTrasporti();
			if (salvataggio) {
				btnEsportaDati.setEnabled(false);
