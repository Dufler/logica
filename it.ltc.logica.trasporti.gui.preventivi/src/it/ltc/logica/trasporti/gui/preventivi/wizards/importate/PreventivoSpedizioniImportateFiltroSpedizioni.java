...
public class PreventivoSpedizioniImportateFiltroSpedizioni extends PaginaWizard {
	
	private static final String titolo = "Preventivo di costo e fatturazione per spedizioni importate";
	private static final String descrizione = "Seleziona i criteri di filtraggio per le spedizioni da simulare.";
	
	private PreventivoSimulazioniImportateController controllerPreventivo;
	...
	private Label lblNonNecessario;
	private Button btnResetCriteri;

	protected PreventivoSpedizioniImportateFiltroSpedizioni() {
	...
	public void aggiungiElementiGrafici(Composite container) {
		...
		lblNonNecessario = new Label(container, SWT.NONE);
		lblNonNecessario.setText("Non \u00E8 necessario compilare tutti i campi, basta inserire solo i filtri di interesse.");
		...
		compositeDate = new CompositeDate(this, container, true);
		compositeDate.setLayoutData(new GridData(SWT.FILL, SWT.TOP, true, false, 1, 1));
		...
		lblNazione.setText("Destinazione: ");
		...
		btnResetCriteri = new Button(container, SWT.NONE);
		btnResetCriteri.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				resetCriteri();
			}
		});
		btnResetCriteri.setText("Reset criteri");
		
	}
	
	private void resetCriteri() {
		compositeDate.resetValues();
		comboCorriere.resetValue();
		comboCodice.resetValue();
		comboServizio.resetValue();
		comboTipo.resetValue();
		btnIndifferente.setSelection(true);
