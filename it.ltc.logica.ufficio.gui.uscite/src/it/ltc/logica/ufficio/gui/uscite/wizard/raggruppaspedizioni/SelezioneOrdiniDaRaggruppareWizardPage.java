...
public class SelezioneOrdiniDaRaggruppareWizardPage extends PaginaWizard {
	
	private final static String title = "Raggruppamento Spedizioni";
	private final static String description = "Seleziona gli ordini da raggruppare in una sola spedizione.";
	
	private final List<OrdineTestata> ordiniDaRaggruppare;
	
	private TabellaOrdiniDaLavorare tabella;
	
	private final Commessa commessa;

	protected SelezioneOrdiniDaRaggruppareWizardPage(Commessa commessa, List<OrdineTestata> ordiniDaRaggruppare) {
		super(title, description);
		this.commessa = commessa;
		this.ordiniDaRaggruppare = ordiniDaRaggruppare;
	}
	...
	public void copyDataToModel() {
		ordiniDaRaggruppare.clear();
		ordiniDaRaggruppare.addAll(tabella.getElementiSelezionati());
	}
	
	@Override
	protected boolean validazioneSpecifica(boolean valid) {
		boolean valido = valid && tabella.getElementiSelezionati().size() > 1;
		return valido;
	}
