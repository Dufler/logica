...
public class ReportListaCompleto extends ReportOrdine {
	...
	public String creaReport(Commessa commessa, RisultatoAssegnazioneOrdine listaDiPrelievo) {
		...
		try {
			...
			//Prendo le righe a prelievo e non disponibili - FIXME Attualmente le prendo tutte ma dovrebbe andare bene così!
//			List<RisultatoAssegnazioneRigaOrdine> listaCompleta = new LinkedList<>();
//			listaCompleta.addAll(listaDiPrelievo.getPrelevabili());
//			listaCompleta.addAll(listaDiPrelievo.getNonPresenti());
			//Raggruppo le righe con lo stesso prodotto, lo stesso collo e la stessa ubicazione.
//			Collection<RisultatoAssegnazioneRigaOrdine> elementi = raggruppaElementi(listaCompleta);
			Collection<RisultatoAssegnazioneRigaOrdine> elementi = raggruppaElementi(listaDiPrelievo.getAssegnazioni());
			//Genero il report vero e proprio nel path di default
			exportPath = getDefaultReportExportPath();
			JasperReportBuilder.buildReportPDF(ReportJasper.ORDINE_LISTA_TUTTI_PDF, exportPath, parameters, elementi);
		} catch (Exception e) {
			exportPath = null;
