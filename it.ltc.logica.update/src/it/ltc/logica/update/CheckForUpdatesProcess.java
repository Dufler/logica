...
//	private static final String repository = "http://ws.services.ltc-logistics.it/logica-update/";
	...
	private File getFeatureListFile() {
		...
		boolean success = controller.download(file.getPath(), repository + featureListFile);
		if (!success) {
	...
	private boolean updateFeature(Feature feature) {
		...
		controller.download(local, repository + url);
	...
	private void updatePlugin(Plugin plugin) {
		...
		if (!pluginJar.exists()) {
			...
			boolean download = controller.download(pluginJar.getPath(), repository + "plugins/" + pluginName);
			if (download) {
