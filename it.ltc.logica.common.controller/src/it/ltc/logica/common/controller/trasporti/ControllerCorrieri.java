...
public class ControllerCorrieri {
	...
	public Corriere getCorriere(String codiceCorriere) {
		Corriere corriere = null;
		for (Corriere c : corrieri.values()) {
			if (c.getCodifica().equals(codiceCorriere)) {
				corriere = c;
				break;
			}
		}
	...
	public List<ServizioCorriere> getServiziPerCorriere(int idCorriere) {
		...
		for (ServizioCorriere servizio : servizi) {
			if (servizio.getCorriere() == idCorriere) {
