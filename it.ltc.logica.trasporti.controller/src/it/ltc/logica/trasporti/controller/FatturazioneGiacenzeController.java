...
public class FatturazioneGiacenzeController extends FatturazioneController {
	...
	private List<Giacenza> getGiacenze() {
		...
		for (Giacenza giacenza : controllerGiacenze.getGiacenze()) {
			...
			if (fatturabile) {
				fatturabile = giacenza.getIdCommessa() == commessa.getId();
				if (fatturabile) {
	...
	public boolean salvaDatiTrasporti() {
		...
		if (idDocumento != -1) {
			...
			for (String codice : modelGiacenze.keySet()) {
				...
				for(SpedizioneModel model : spedizioni) {
					//Se è stata annullata passo alla successiva.
					if (model.isFatturazioneAnnullata())
						continue;
					int idRiferimento = model.getGiacenza().getId();
					Calcolo preventivo = model.getPreventivoRicavo();
					...
					for (VoceCalcolata voce : preventivo.getVoci()) {
						...
						nuovaVoce.setIdRiferimento(idRiferimento);
					...
					elemento.setRiferimento(idRiferimento);
