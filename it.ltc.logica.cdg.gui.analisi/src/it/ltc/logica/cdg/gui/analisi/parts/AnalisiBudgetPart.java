...
public class AnalisiBudgetPart {
	...
	private HashMap<String, Double> totaliRicavoBudget;
	private HashMap<String, Double> totaliCostoBudget;
	
	@Inject
	public AnalisiBudgetPart() {
		...
		btnCalcolaCDG.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (sede == null || inizio == null || fine == null) {
					...
					totaliRicavoBudget = controllerAnalisi.elaboraRicaviBudget(sede, inizio, fine); 
					totaliCostoBudget = controllerAnalisi.elaboraCostiBudget(sede, inizio, fine);
					comboCommessa.setItems(analisi);
					elaboraAnalisiControlloCDG(analisi, totaliRicavoBudget, totaliCostoBudget);
				}
			}
		});
		...
		btnFiltra.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (analisiCommessa != null) {
					HashMap<String, Double> totaliRicavoBudget = controllerAnalisi.elaboraRicaviBudget(analisiCommessa.getCommessa(), inizio, fine); 
					HashMap<String, Double> totaliCostoBudget = controllerAnalisi.elaboraCostiBudget(analisiCommessa.getCommessa(), inizio, fine);
					elaboraAnalisiControlloCDGPerCommessa(analisiCommessa, totaliRicavoBudget, totaliCostoBudget);
				} else {
	...
	private void elaboraGrafico(String titolo, HashMap<String, Double> totaliRicavoPerNome, HashMap<String, Double> totaliCostoPerNome, HashMap<String, Double> totaliRicavoBudget, HashMap<String, Double> totaliCostoBudget) {
		...
		DatiGraficoSemplice dati = new DatiGraficoSemplice(titolo, "", "Euro");
		//Aggiungo i totali al grafico
		for (String nome : totaliRicavoPerNome.keySet()) {
			double valore = totaliRicavoPerNome.get(nome);
			dati.aggiungiValore(valore, nome, "Ricavo");
		}
		for (String nome : totaliRicavoBudget.keySet()) {
			double valore = totaliRicavoBudget.get(nome);
			dati.aggiungiValore(valore, nome, "Ricavo Budget");
		}
		for (String nome : totaliCostoPerNome.keySet()) {
			double valore = totaliCostoPerNome.get(nome);
			dati.aggiungiValore(valore, nome, "Costo");
		}
		for (String nome : totaliCostoBudget.keySet()) {
			double valore = totaliCostoBudget.get(nome);
			dati.aggiungiValore(valore, nome, "Costo Budget");
		}
		grafico.costruisciGraficoABarreOrizzontaliComposte(dati);
	}
	
	private void elaboraTotali(HashMap<String, Double> totaliRicavoPerNome,	HashMap<String, Double> totaliCostoPerNome, HashMap<String, Double> totaliRicavoBudget,	HashMap<String, Double> totaliCostoBudget) {
		double totaleCosti = 0;
		double totaleRicavi = 0;
		double totaleCostiBudget = 0;
		double totaleRicaviBudget = 0;
		//Aggiungo i totali al grafico
		for (String nome : totaliRicavoPerNome.keySet()) {
			double valore = totaliRicavoPerNome.get(nome);
			totaleRicavi += valore;
		...
		for (String nome : totaliRicavoBudget.keySet()) {
			double valore = totaliRicavoBudget.get(nome);
			totaleRicaviBudget += valore;
		}
		for (String nome : totaliCostoBudget.keySet()) {
			double valore = totaliCostoBudget.get(nome);
			totaleCostiBudget += valore;
		}
		String risultato = totaleRicavi > totaleCosti ? "Utile : +" : "Perdita : -";
		lblTotaleCostiRealiCDG.setText("Totale Costi : " + formatEuro.format(totaleCosti) + ", Budget Costi : " + formatEuro.format(totaleCostiBudget));
		lblTotaleRicaviIpoteticiCDG.setText("Totale Ricavi : " + formatEuro.format(totaleRicavi) + ", Budget Ricavi : " + formatEuro.format(totaleRicaviBudget));
		lblTotaleScostamentoCDG.setText(risultato + formatEuro.format(Math.abs(totaleRicavi - totaleCosti)) + ", Utile ipotizzato : " + formatEuro.format(Math.abs(totaleRicaviBudget - totaleCostiBudget)));
	}
	
	private void elaboraMappaCosti(AnalisiCommessa analisiCommessa, HashMap<String, Double> totaliCostoPerNome) {
		HashMap<Integer, RiepilogoGiornalieroCDG> riepiloghi = analisiCommessa.getRiepilogoPerGiorno();
		for (RiepilogoGiornalieroCDG riepilogo : riepiloghi.values()) {
			for (ElementoBilancioCDG elemento : riepilogo.getElementi()) {
				double totale = totaliCostoPerNome.containsKey(elemento.getOrigine()) ? totaliCostoPerNome.get(elemento.getOrigine()) : 0.0;
				totale += elemento.getCostoReale();
				totaliCostoPerNome.put(elemento.getOrigine(), totale);
			}
		}
	}
	...
	private void checkPresenzaBudget(HashMap<String, Double> totaliRicavoBudget, HashMap<String, Double> totaliCostoBudget) {
		if (totaliRicavoBudget.containsKey(ControllerAnalisiCDG.BUDGET_NON_PRESENTE) || totaliCostoBudget.containsKey(ControllerAnalisiCDG.BUDGET_NON_PRESENTE)) {
			DialogMessaggio.openWarning("Simulazione incompleta", "Attenzione, non sono presenti tutti i budget necessari per l'intervallo di tempo indicato.");
		}
	}
	
	private void elaboraAnalisiControlloCDG(List<AnalisiCommessa> analisi, HashMap<String, Double> totaliRicavoBudget, HashMap<String, Double> totaliCostoBudget) {
		// Ripulisco tutto
		ripulisciComposite();
		...
		// Elaboro il grafico
		elaboraGrafico(titolo, totaliRicavoPerNome, totaliCostoPerNome, totaliRicavoBudget, totaliCostoBudget);

		// Elaboro i totali
		elaboraTotali(totaliRicavoPerNome, totaliCostoPerNome, totaliRicavoBudget, totaliCostoBudget);

		// Aggiorno i composite
		aggiornaComposite();
		
		//Se necessario avverto l'utente che alcuni budget non sono stati inseriti
		checkPresenzaBudget(totaliRicavoBudget, totaliCostoBudget);
	}
	
	private void elaboraAnalisiControlloCDGPerCommessa(AnalisiCommessa analisiCommessa, HashMap<String, Double> totaliRicavoBudget, HashMap<String, Double> totaliCostoBudget) {
		// Ripulisco tutto
		ripulisciComposite();
